...
function detectCategories(query) {
  const normalized = normalize(query); const detected = new Set();
  for (const [category, aliases] of Object.entries(SEARCH_CONFIG.categoryAliases ?? {})) if ((aliases ?? []).map(normalize).some((alias) => phrasePresent(normalized, alias))) detected.add(category.toUpperCase());
  return detected;
}
...
function editDistance(a, b) {
  ...
  for (let i = 1; i <= a.length; i += 1) { const current = [i]; for (let j = 1; j <= b.length; j += 1) current[j] = Math.min(previous[j - 1] + (a[i - 1] === b[j - 1] ? 0 : 1), current[j - 1] + 1, previous[j] + 1); previous = current; }
...
function score(item, query) {
  ...
  const fields = fieldsFor(item); const variants = expandQuery(query); const detectedCategories = detectCategories(query); const detectedIntents = detectIntents(query); const primary = fields.question || fields.title; const primaryTokens = tokenSet(primary); const uniqueTokenCount = baseTokens.length;
  const matchedPrimary = coverage(baseTokens, primary); const matchedTitle = coverage(baseTokens, fields.title); const matchedSection = coverage(baseTokens, fields.section); const matchedSourceTitle = coverage(baseTokens, fields.sourceTitle); const matchedAliases = coverage(baseTokens, fields.aliases.join(' ')); const matchedKeywords = coverage(baseTokens, fields.keywords.join(' '));
...
function diversifyScoredResults(scored, limit = 20) {
  ...
  while (remaining.length && selected.length < Math.min(limit, scored.length)) {
    ...
    const [picked] = remaining.splice(bestIndex, 1); selected.push(picked); const sourceId = picked.item.source?.id ?? ''; const category = String(picked.item.category ?? '').toUpperCase(); sourceCounts.set(sourceId, (sourceCounts.get(sourceId) ?? 0) + 1); categoryCounts.set(category, (categoryCounts.get(category) ?? 0) + 1);
  }
...
export function searchKnowledge(knowledge, query, { minScore = 24, diversify = true } = {}) {
  ...
  const scored = knowledge.map((item) => ({ item, score: score(item, query) })).filter(({ score: itemScore }) => itemScore >= threshold).sort((a, b) => b.score - a.score || sourcePriority(b.item) - sourcePriority(a.item) || (a.item.question ?? a.item.title ?? '').localeCompare(b.item.question ?? b.item.title ?? ''));
