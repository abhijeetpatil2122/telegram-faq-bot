...
function fieldsFor(item) {
  return {
    question: normalize(item.question ?? ''), title: normalize(item.title ?? ''), section: normalize(item.section ?? ''),
    sourceTitle: normalize(item.source?.title ?? ''), aliases: (item.aliases ?? []).map(normalize).filter(Boolean),
    keywords: (item.keywords ?? []).map(normalize).filter(Boolean), category: normalize(item.category ?? ''), audience: normalize(item.metadata?.audience ?? '')
  };
}
...
function editDistance(a, b) {
  ...
  for (let i = 1; i <= a.length; i += 1) {
    const current = [i];
    for (let j = 1; j <= b.length; j += 1) current[j] = Math.min(previous[j - 1] + (a[i - 1] === b[j - 1] ? 0 : 1), current[j - 1] + 1, previous[j] + 1);
    previous = current;
  }
...
function fuzzySimilarity(a, b) {
  ...
  const distance = editDistance(a, b); if (distance > 2) return 0;
...
function score(item, query) {
  const normalizedQuery = normalize(query); const baseTokens = queryTokens(query); if (!normalizedQuery || !baseTokens.length) return 0;
  const fields = fieldsFor(item); const variants = expandQuery(query); const detectedCategories = detectCategories(query); const detectedIntents = detectIntents(query);
  const primary = fields.question || fields.title; const primaryTokens = tokenSet(primary); const uniqueTokenCount = baseTokens.length;
  const matchedPrimary = coverage(baseTokens, primary); const matchedTitle = coverage(baseTokens, fields.title); const matchedSection = coverage(baseTokens, fields.section);
  const matchedSourceTitle = coverage(baseTokens, fields.sourceTitle); const matchedAliases = coverage(baseTokens, fields.aliases.join(' ')); const matchedKeywords = coverage(baseTokens, fields.keywords.join(' '));
  const allSearchFields = [fields.question, fields.title, fields.section, fields.sourceTitle, ...fields.aliases, ...fields.keywords]; let points = 0;
  if (fields.question === normalizedQuery) points += 1000; if (fields.title === normalizedQuery) points += 900; if (fields.section === normalizedQuery) points += 700; if (fields.sourceTitle === normalizedQuery) points += 650;
  for (const variant of variants) { if (variant === normalizedQuery) continue; if (fields.question === variant) points += 520; if (fields.title === variant) points += 460; if (fields.aliases.some((alias) => alias === variant)) points += 430; }
  if (containsPhraseAny(fields.question, variants)) points += 420; if (containsPhraseAny(fields.title, variants)) points += 360; if (containsPhraseAny(fields.section, variants)) points += 220; if (containsPhraseAny(fields.sourceTitle, variants)) points += 260;
  if (fields.aliases.some((field) => containsPhraseAny(field, variants))) points += 300; if (fields.keywords.some((field) => containsPhraseAny(field, variants))) points += 180;
  if (fields.question.startsWith(normalizedQuery)) points += 260; else if (fields.title.startsWith(normalizedQuery)) points += 220; else if (fields.section.startsWith(normalizedQuery)) points += 140; else if (fields.sourceTitle.startsWith(normalizedQuery)) points += 120;
  if (fields.question.includes(normalizedQuery)) points += 140; if (fields.title.includes(normalizedQuery)) points += 120; if (fields.section.includes(normalizedQuery)) points += 80; if (fields.sourceTitle.includes(normalizedQuery)) points += 70;
  points += matchedPrimary * 90 + matchedTitle * 65 + matchedSection * 35 + matchedSourceTitle * 55 + matchedAliases * 45 + matchedKeywords * 20;
...
function effectiveMinScore(query, configured) {
  ...
  if (count <= 1) return Math.max(18, base - 6); if (count === 2) return Math.max(20, base - 3); if (count >= 5) return base + 3; return base;
}
function diversifyScoredResults(scored, limit = 20) {
  ...
  while (remaining.length && selected.length < Math.min(limit, scored.length)) {
    let bestIndex = 0; let bestAdjusted = -Infinity;
    for (let index = 0; index < remaining.length; index += 1) {
      const candidate = remaining[index]; const sourceId = candidate.item.source?.id ?? ''; const category = String(candidate.item.category ?? '').toUpperCase(); let adjusted = candidate.score;
      if (selected.length > 0) { adjusted -= Math.min(36, (sourceCounts.get(sourceId) ?? 0) * 18); adjusted -= Math.min(8, (categoryCounts.get(category) ?? 0) * 4); }
      const currentBest = remaining[0].score; if (candidate.score < currentBest * 0.72) adjusted -= 40;
      if (adjusted > bestAdjusted) { bestAdjusted = adjusted; bestIndex = index; }
    }
    const [picked] = remaining.splice(bestIndex, 1); selected.push(picked); const sourceId = picked.item.source?.id ?? ''; const category = String(picked.item.category ?? '').toUpperCase();
    sourceCounts.set(sourceId, (sourceCounts.get(sourceId) ?? 0) + 1); categoryCounts.set(category, (categoryCounts.get(category) ?? 0) + 1);
  }
  return [...selected, ...remaining];
}
export function searchKnowledge(knowledge, query, { minScore = 24, diversify = true } = {}) {
  if (!normalize(query)) return [...knowledge]; const threshold = effectiveMinScore(query, minScore);
  const scored = knowledge.map((item) => ({ item, score: score(item, query) })).filter(({ score: itemScore }) => itemScore >= threshold)
  ...
  const ranked = diversify ? diversifyScoredResults(scored) : scored; return ranked.map(({ item }) => item);
}
