...
function safeUrl(value, base) {
  const raw = String(value ?? '').trim(); if (!raw || /^(?:javascript|data|vbscript):/i.test(raw)) return null;
  try { const url = new URL(raw, base); return ALLOWED_PROTOCOLS.has(url.protocol) ? url.toString() : null; } catch { return null; }
}
function keywordsFor(title, answer) {
  return [...new Set(`${normalize(title)} ${normalize(answer).slice(0,5000)}`.split(' ').filter((x) => (x.length >= 3 || ['api','app','bot','bots','faq','otp','url'].includes(x)) && !STOPWORDS.has(x)))].slice(0, 100);
}
function serialize(node, $, sourceUrl, context = {}) {
  if (node.type === 'text') return escapeHtml(context.pre ? node.data : String(node.data).replace(/\\s+/g, ' '));
  if (node.type !== 'tag') return '';
  const tag = String(node.name || '').toLowerCase();
  if (['script','style','noscript','template','svg','nav','header','footer','aside'].includes(tag)) return '';
  if (tag === 'br') return '\\n';
  const inner = $(node).contents().toArray().map((child) => serialize(child, $, sourceUrl, { ...context, pre: tag === 'pre' })).join('');
...
function extract(html, source) {
  const { $, root } = loadDoc(html); const nodes = contentNodes(root); const headings = [];
  for (let i=0;i<nodes.length;i++) { const n=nodes[i]; if (/^h[1-6]$/i.test(n.name)) headings.push({ index:i, level:Number(n.name.slice(1)), text:cleanTitle($(n).text()) }); }
  const items=[]; const faq = source.kind === 'faq';
  for (let h=0;h<headings.length;h++) {
    const heading=headings[h];
    if (faq && !/^Q:/i.test(heading.text) && !/^Q\\s*:/i.test($(nodes[heading.index]).text())) continue;
    if (!faq && heading.level < 2) continue;
    let end=nodes.length; for (let j=h+1;j<headings.length;j++) if (headings[j].level <= heading.level) { end=headings[j].index; break; }
...
async function fetchSource(source) {
  ...
  try {
    ...
    const length=Number(res.headers.get('content-length')||0); if(length && length>crawlerConfig.maxSourceBytes) throw new Error(`source exceeds ${crawlerConfig.maxSourceBytes} bytes`);
    const html=await res.text(); if(Buffer.byteLength(html,'utf8')>crawlerConfig.maxSourceBytes) throw new Error(`source exceeds ${crawlerConfig.maxSourceBytes} bytes`);
    return html;
...
const allItems=[]; const sourceMeta=[]; const state={schemaVersion:1,generatedAt:new Date().toISOString(),sources:{}};
for(const source of indexedSources){
  process.stdout.write(`Crawling ${source.id}... `);
  let html; try { html=await fetchSource(source); } catch(error) { throw new Error(`Fetch failed for ${source.id}: ${error.message}`); }
  ...
  allItems.push(...items); sourceMeta.push({id:source.id,title:source.title,url:source.url,kind:source.kind,audience:source.audience,category:source.category,priority:source.priority});
...
allItems.sort((a,b)=>a.id.localeCompare(b.id)); sourceMeta.sort((a,b)=>a.id.localeCompare(b.id));
const dataset={schemaVersion:3,generatedAt:new Date().toISOString(),generator:'scripts/crawl/index.mjs',sources:sourceMeta,items:allItems};
