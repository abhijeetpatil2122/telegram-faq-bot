...
function extract(html, source) {
  ...
  for (let i = 0; i < nodes.length; i += 1) { const n = nodes[i]; if (/^h[1-6]$/i.test(n.name)) headings.push({ index: i, level: Number(n.name.slice(1)), raw: cleanText($(n).text()), text: cleanTitle($(n).text()) }); }
  const items = []; const faq = source.kind === 'faq';
  for (let h = 0; h < headings.length; h += 1) {
    const heading = headings[h];
    if (faq && !/^Q\s*:/i.test(heading.raw)) continue;
    if (!faq && heading.level < 2) continue;
    let end = nodes.length; for (let j = h + 1; j < headings.length; j += 1) if (headings[j].level <= heading.level) { end = headings[j].index; break; }
...
async function fetchSource(source) {
  ...
  try { const res = await fetch(source.url, { headers: { 'user-agent': crawlerConfig.userAgent, accept: 'text/html,application/xhtml+xml' }, signal: controller.signal, redirect: 'follow' }); if (!res.ok) throw new Error(`${res.status} ${res.statusText}`); const length = Number(res.headers.get('content-length') || 0); if (length && length > crawlerConfig.maxSourceBytes) throw new Error(`source exceeds ${crawlerConfig.maxSourceBytes} bytes`); const html = await res.text(); if (Buffer.byteLength(html, 'utf8') > crawlerConfig.maxSourceBytes) throw new Error(`source exceeds ${crawlerConfig.maxSourceBytes} bytes`); return html; } finally { clearTimeout(timer); }
}
