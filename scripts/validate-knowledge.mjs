...
if (data.schemaVersion !== 2) throw new Error('Unsupported knowledge schemaVersion');
...
const allowedTags = new Set([
  'a', 'b', 'strong', 'i', 'em', 'u', 'ins', 's', 'strike', 'del', 'code', 'mark', 'sub', 'sup',
  'tg-spoiler', 'tg-reference', 'tg-emoji', 'tg-time', 'tg-math', 'h1', 'h2', 'h3', 'h4', 'h5',
  'h6', 'p', 'pre', 'footer', 'hr', 'ul', 'ol', 'li', 'blockquote', 'aside', 'figure', 'figcaption',
  'cite', 'table', 'caption', 'tr', 'th', 'td', 'details', 'summary', 'input'
]);

for (const item of data.items) {
  if (!item.id || ids.has(item.id)) throw new Error(`Duplicate/missing item id: ${item.id ?? '(missing)'}`);
  if (!item.title || !item.answer || !item.answer_html) throw new Error(`Incomplete item: ${item.id}`);
  ...
  if (item.answer_html.length < 20) throw new Error(`Rich answer too short for ${item.id}`);

  const tags = [...item.answer_html.matchAll(/<\/?([A-Za-z0-9:-]+)/g)].map((match) => match[1].toLowerCase());
  for (const tag of tags) {
    if (!allowedTags.has(tag)) throw new Error(`Unsupported Rich HTML tag <${tag}> in ${item.id}`);
  }
